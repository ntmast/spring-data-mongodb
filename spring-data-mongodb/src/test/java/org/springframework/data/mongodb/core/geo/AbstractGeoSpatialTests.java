...
public abstract class AbstractGeoSpatialTests {
	...
	public void geoWithinCenter() {
		...
		Circle circle = new Circle(-73.99171, 40.738868, 0.01);
		List<Venue> venues = template.find(query(where("location").geoWithin(circle)), Venue.class);
		assertThat(venues.size(), is(7));
	}
	...
	public void geoWithinCenterSphere() {
		...
		Circle circle = new Circle(-73.99171, 40.738868, 0.003712240453784);
		List<Venue> venues = template.find(query(where("location").geoWithinSphere(circle)), Venue.class);
		assertThat(venues.size(), is(11));
	}
	...
	public void geoWithinBox() {
		...
		Box box = new Box(new Point(-73.99756, 40.73083), new Point(-73.988135, 40.741404));
		List<Venue> venues = template.find(query(where("location").geoWithin(box)), Venue.class);
		assertThat(venues.size(), is(4));
	}
	...
	public void geoWithinPolygon() {
		...
		List<Venue> venues = template.find(query(where("location").geoWithin(polygon)), Venue.class);
		assertThat(venues.size(), is(4));
	}
